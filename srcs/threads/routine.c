...
/*   Updated: 2024/10/07 14:36:52 by eviala           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** {
** 	t_data *(data) = philo->data;
** 	pthread_mutex_lock(&data->dining_room_mutex);
** 	while (data->philosophers_eating >= data->nb_philos / 2)
** 	{
** 		pthread_mutex_unlock(&data->dining_room_mutex);
** 		if (is_dead(philo))
** 			return ;
** 		usleep(10);
** 		pthread_mutex_lock(&data->dining_room_mutex);
...
void	*td_routine(void *args)
{
	...
	if (philo->index_tab % 2)
		ft_usleep(philo->data->time_te, data);
	while (1)
	{
		if (is_dead(philo))
			return (NULL);
